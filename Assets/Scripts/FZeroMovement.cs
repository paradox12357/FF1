...
public class FZeroMovement : MonoBehaviour
{
    ...
    //public float hover_range_high = 3.5f;//Range at which the ship should hover in (high)
    //public float hover_range_low = 2.5f;
    //public float rising = 0f;            //whether or not the ship is rising (hover)
    ...
    void FixedUpdate()
    {
        ...
        yaw += turn_speed * Time.deltaTime * (((Input.GetAxis("Horizontal") < 0.25) && (Input.GetAxis("Horizontal") > -0.25)) ? 0f : Input.GetAxis("Horizontal"));//Turn if the joystick isn't in the deadzone
